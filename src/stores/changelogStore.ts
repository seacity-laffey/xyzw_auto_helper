...
type ChangelogType = "major" | "minor" | "patch" | "hotfix";
...
export const useChangelogStore = defineStore("changelog", () => {
   ...
   */
  const addChangelog = (changelog: ChangelogEntry) => {
   ...
   */
  const updateChangelog = (version: string, updates: Partial<ChangelogEntry>) => {
    const index = changelogs.value.findIndex((log) => log.version === version);
    if (index !== -1) {
      const existingChangelog = changelogs.value[index];
      if (!existingChangelog) return;
   ...
   * @param {String} version - 版本号
   */
  const deleteChangelog = (version: string) => {
    const index = changelogs.value.findIndex((log) => log.version === version);
    if (index !== -1) {
   ...
   */
  const getVersionDiff = (fromVersion: string, toVersion: string) => {
   ...
   */
  const hasNewVersion = (currentVersion: string) => {
