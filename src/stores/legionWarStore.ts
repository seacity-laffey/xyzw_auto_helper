...
interface BattlefieldData {
  legionInfo: Record<string, LegionInfo>;
  [key: string]: unknown;
}

interface BattlefieldResponse {
  info?: {
    battlefieldId?: string | number | null;
    sid?: string | number;
  };
}

interface LegionDetailResponse {
  legionData?: unknown;
  info?: unknown;
}

interface LegionWarMessage {
  cmd?: string;
  rawData?: unknown;
  [key: string]: unknown;
}

type LegionDetails = Record<string, unknown>;
type LegionWarClientWithEvents = XyzwLegionWarWebSocketClient & {
  onConnect?: (() => void) | null;
  onDisconnect?: ((event: CloseEvent | unknown) => void) | null;
...
export const useLegionWarStore = defineStore("legionWar", () => {
  ...
  const battlefieldId = ref<string | number | null>(null);
  const validData = ref<BattlefieldData | null>(null);
  const legionDetails = ref<LegionDetails>({});
  ...
  // WebSocket 实例
  let legionWarWebSocket: LegionWarClientWithEvents | null = null;
  ...
  const connect = async () => {
    ...
    try {
      ...
      const getbattlefield = (await tokenStore.sendMessageWithPromise(
        ...
        {},
        10000,
      )) as BattlefieldResponse | null;

      if (!getbattlefield || !getbattlefield.info) {
        throw new Error("无法获取战场信息");
      }

      battlefieldId.value = getbattlefield.info.battlefieldId ?? null;
      ...
      legionWarWebSocket.setMessageListener((msg: LegionWarMessage) => {
        ...
        if (cmd.includes("war_getbattlefieldinfo")) {
          ...
          if (extracted) {
            ...
            // 获取俱乐部详情以获取公告
            const legionInfo = extracted.legionInfo as Record<string, LegionInfo>;
            Object.values(legionInfo).forEach((legion) => {
              // 如果还没有该俱乐部的详情，则发送请求
              const legionId = String(legion.id);
              if (!legionDetails.value[legionId]) {
                fetchLegionDetail(legionId);
              }
            });
          }
        }
  ...
  const fetchLegionDetail = async (legionId: string) => {
