...
export const useGameRolesStore = defineStore("gameRoles", () => {
  // 状态
  const gameRoles = ref<GameRole[]>([]);
  const isLoading = ref(false);
  const selectedRole = ref<GameRole | null>(null);
  ...
  const fetchGameRoles = async () => {
    try {
      ...
      if (savedRoles) {
        try {
          gameRoles.value = JSON.parse(savedRoles) as GameRole[];
        } catch (error) {
  ...
  const addGameRole = async (roleData: NewGameRole): Promise<StoreResult> => {
    try {
      isLoading.value = true;
  ...
  const updateGameRole = async (
    roleId: string,
    roleData: GameRoleUpdate,
  ): Promise<StoreResult> => {
    try {
      isLoading.value = true;
      ...
      if (index !== -1) {
        const existingRole = gameRoles.value[index];
        if (!existingRole) {
  ...
  const deleteGameRole = async (roleId: string): Promise<StoreResult> => {
    try {
      isLoading.value = true;
  ...
  const initGameRoles = () => {
    ...
    if (cachedRoles) {
      try {
        gameRoles.value = JSON.parse(cachedRoles) as GameRole[];
      } catch (error) {
    ...
    if (cachedSelectedRole) {
      try {
        selectedRole.value = JSON.parse(cachedSelectedRole) as GameRole;
      } catch (error) {
