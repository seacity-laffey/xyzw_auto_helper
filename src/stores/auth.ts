...
export const useAuthStore = defineStore("auth", () => {
  ...
  // 登录 - 移除API调用，使用本地认证
  const login = async (credentials: LoginCredentials): Promise<StoreResult> => {
    try {
  ...
  // 注册 - 移除API调用，使用本地注册
  const register = async (userInfo: RegisterUserInfo): Promise<StoreResult> => {
    try {
      isLoading.value = true;
  ...
  const fetchUserInfo = async () => {
    try {
      ...
      if (savedUser) {
        try {
          user.value = JSON.parse(savedUser) as LocalUser;
          return true;
        } catch (error) {
