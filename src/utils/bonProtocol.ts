...
import type { GameCommandParams, GamePacket } from "@/types/gameProtocol";
...
type ProtoDataCtor<T> = {
  new (...args: never[]): T;
  name: string;
};

interface SendMessageMeta<T = unknown> {
  respType?: ProtoDataCtor<T>;
}
...
export class ProtoMsgLegion {
  ...
  _rawData: unknown;
  _data: unknown;
  _t: { name: string; ctor: ProtoDataCtor<unknown> } | undefined;
  _sendMsg: SendMessageMeta | undefined;
  rtt: number;

  constructor(raw: GamePacket) {
    if (raw?.cmd) {
  ...
  /** 指定数据类型 */
  setDataType(t?: ProtoDataCtor<unknown>) {
    if (t) this._t = { name: t.name ?? "Anonymous", ctor: t };
    return this;
  }

  /** 配置"请求"对象，让 respType 自动对齐 */
  setSendMsg(msg: SendMessageMeta) {
    this._sendMsg = msg;
    return this.setDataType(msg.respType);
  }

  /** 将 rawData 反序列化为业务对象 */
  getData<T = unknown>(clazz?: ProtoDataCtor<T>): T {
...
/** 注册器 */
function register(name: string, impl: EncImpl) {
...
/** 对外：按名称取加解密器；找不到则用默认 */
export function getEnc(name: string): EncImpl {
...
export function parse(buf: ArrayBuffer, enc: EncImpl, isLegion = false) {
  if (!isLegion) {
...
function parseLegion(buf: ArrayBuffer, enc: EncImpl, isLegion: boolean) {
  const u8 = new Uint8Array(buf);
  const plain = enc.decrypt(u8);
  const raw = bon.decode(plain);
...
export const GameMessages = {
  ...
  // 获取角色信息
  getRoleInfo: (ack = 0, seq = 0, params: GameCommandParams = {}) => ({
  ...
  // 获取数据包版本
  getDataBundleVer: (ack = 0, seq = 0, params: GameCommandParams = {}) => ({
  ...
  // 购买金币
  buyGold: (ack = 0, seq = 0, params: GameCommandParams = {}) => ({
    ack,
    body: encode(
      {
        buyNum: 1,
  ...
  // 签到奖励
  signInReward: (ack = 0, seq = 0, params: GameCommandParams = {}) => ({
    ack,
    body: encode(
      {
        ...params,
  ...
  // 领取每日任务奖励
  claimDailyReward: (ack = 0, seq = 0, params: GameCommandParams = {}) => ({
    ack,
    body: encode(
      {
        rewardId: 0,
...
export const g_utils = {
  getEnc,
  encode: (obj: unknown, encName = "x") => encode(obj, getEnc(encName)),
  parse: (data: ArrayBuffer, encName = "auto", isLegion = false) =>
    parse(data, getEnc(encName), isLegion),
  bon, // 添加BON编解码器
};
...
export const bonProtocol = {
  ...
  parseMessage: (messageData: string | GamePacket) => {
    try {
      let message;
