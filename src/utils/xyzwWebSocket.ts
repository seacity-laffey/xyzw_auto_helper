...
interface BonUtils {
  bon?: {
    encode?: (value: GameCommandParams) => unknown;
...
interface XyzwWebSocketClientOptions {
  url: string;
  utils?: BonUtils;
  heartbeatMs?: number;
}
...
export class CommandRegistry {
  ...
  constructor(encoder: BonUtils, enc: unknown) {
  ...
  /** 注册命令 */
  register(cmd: string, defaultBody: GameCommandParams = {}) {
...
/** 预注册游戏命令 */
export function registerDefaultCommands(reg: CommandRegistry) {
  const registry = reg
    .registerHeartbeat()
    // 角色/系统
...
export class XyzwWebSocketClient {
  ...
  seq: number;
  sendQueue: SendTask[];
  sendQueueTimer: ReturnType<typeof setInterval> | null;
  ...
  promises: Record<string, PromiseState>;
  registry: CommandRegistry;
  onConnect: (() => void) | null;
  onDisconnect: ((event: CloseEvent) => void) | null;
  onError: ((event: Event) => void) | null;

  constructor({ url, utils, heartbeatMs = 5000 }: XyzwWebSocketClientOptions) {
    this.url = url;
    ...
    this.promises = Object.create(null) as Record<string, PromiseState>;
  ...
  /** 注册消息回调 */
  setMessageListener(fn: (packet: GamePacket) => void) {
    this.messageListener = fn;
  }

  /** 控制台消息开关 */
  setShowMsg(val: boolean) {
    this.showMsg = !!val;
  }
  ...
  async debounceSend(cmd: string, ...args: [GameCommandParams?, number?]) {
  ...
  /** 获取角色信息 */
  getRoleInfo(params: GameCommandParams = {}) {
    return this.sendWithPromise("role_getroleinfo", params);
  }

  /** 获取数据版本 */
  getDataBundleVersion(params: GameCommandParams = {}) {
  ...
  /** 处理 Promise 响应 */
  _handlePromiseResponse(packet: GamePacket) {
