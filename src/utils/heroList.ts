...
interface HeroArtifact {
  artifactId: number;
  appendSkill?: Array<{ skillId: number }>;
}

interface PearlSlotGroup {
  artifactId: number;
  slotMap?: Record<string, { colorId: number; [key: string]: unknown }>;
}

interface HeroFillSource {
  heroes?: Record<string, HeroArtifact>;
  pearlMap?: Record<string, PearlSlotGroup>;
}

interface HeroFillEntry {
  FishInfo?: { name: string };
  artifactId: number;
  PearlSkill?: { name: string };
  slotMap?: Array<Record<string, unknown>>;
}
...
export const LINEUP_RULES: LineupRule[] = [
  {
    name: "吴国",
    required: [106, 111],
...
export const getLineupType = (heroList: LineupHero[]) => {
  const ids = new Set(heroList.map((h) => parseInt(String(h.heroId))));
  ...
  for (const rule of LINEUP_RULES) {
    ...
    // 检查二选一/多选一武将
    if (rule.oneOf) {
      const oneOfMatch = rule.oneOf.every((group: number[]) =>
        group.some((id: number) => has(id)),
      );
      if (!oneOfMatch) {
        continue;
      }
...
//鱼珠字典
const PearlMap: Record<number, { name: string }> = {
  1033007: { name: "碎盾" },
  1033008: { name: "冥想" },
  1033009: { name: "定心" },
  1033010: { name: "冰清" },
...
const FishMap: Record<number, { name: string }> = {
  1201: { name: "龙鱼·幽影" },
  1202: { name: "龙鱼·青龙" },
  1203: { name: "龙鱼·火镰" },
  1204: { name: "龙鱼·无双" },
...
//洗练颜色
const color: Record<number, { color: string; value: string }> = {
...
export const formatWeapon = (id: number | string) => {
  return weapon[Number(id)];
};
 ...
 * @returns
 */
export const HeroFillInfo = (obj: HeroFillSource) => {
  const temp: Record<number, HeroFillEntry> = {};
  // 检查 obj 和 obj.heroes 是否存在
  if (obj && obj.heroes && typeof obj.heroes === "object") {
    Object.values(obj.heroes).forEach((hero: HeroArtifact) => {
      temp[hero.artifactId] = {
        FishInfo: FishMap[Number((hero.artifactId + "").substring(0, 4))],
        artifactId: hero.artifactId,
      };
      // 获取鱼珠技能信息，先检查 appendSkill 是否存在
      if (hero.appendSkill && Array.isArray(hero.appendSkill)) {
        hero.appendSkill.forEach((item: { skillId: number }) => {
          const pearl = PearlMap[item.skillId];
          const entry = temp[hero.artifactId];
          if (pearl && entry) {
            entry.PearlSkill = pearl;
          }
        });
      }
    });
  ...
  if (obj && obj.pearlMap && typeof obj.pearlMap === "object") {
    Object.values(obj.pearlMap).forEach((item: PearlSlotGroup) => {
      const entry = temp[item.artifactId];
      if (entry) {
        // 检查 item.slotMap 是否存在
        if (item.slotMap && typeof item.slotMap === "object") {
          entry.slotMap = Object.values(item.slotMap).map(
            (slot) => {
...
export const getTechType = (researchId: number | string) => {
