...
/** Decode one login buffer and request its roles without dropping ArrayBuffer data. */
export async function loadRoleBinData(buffer: ArrayBuffer, fetchRoles: (buffer: ArrayBuffer) => Promise<string>) {
  const payload = getBinPayload(buffer);
  const response = JSON.parse(await fetchRoles(buffer));
  if (!response || typeof response !== "object")
    throw new Error("服务器角色列表格式无效");
  return { payload, roles: Object.values(response) };
}
