...
const queryPromise = (async () => {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    try {
      ...
      const data = (await response.json()) as StudyQuestionItem[];
      return data;
    } catch (error) {
      // try next candidate
 ...
 */
export async function loadQuestionsData(): Promise<StudyQuestionItem[]> {
  return queryPromise;
}
 ...
 */
export async function findAnswer(question: string) {
  try {
