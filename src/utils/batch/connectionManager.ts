...
interface BattleVersionResponse {
  battleData?: {
    version?: number;
  };
}
...
type ArenaTargets = Record<string, unknown> & {
  rankList?: ArenaTarget[];
  roleList?: ArenaTarget[];
  targets?: ArenaTarget[];
  targetList?: ArenaTarget[];
  list?: ArenaTarget[];
  roleId?: number;
  id?: number;
};
...
export function createConnectionManager({
  ...
  const ensureConnection = async (
    ...
    if (!connected) {
      ...
      if (!connected && maxRetries > 0) {
        ...
        if (!refreshedToken) {
          throw new Error(`Token not found after refresh: ${tokenId}`);
        }
        tokenStore.createWebSocketConnection(
          tokenId,
    ...
    try {
      ...
      addLog({
        time: new Date().toLocaleTimeString(),
        message: `初始化数据失败: ${getErrorMessage(e)}`,
        type: "warning",
      });
    }
   ...
   */
  const closeConnection = (tokenId: string, tokenName: string) => {
    tokenStore.closeWebSocketConnection(tokenId);
    releaseConnectionSlot();
    addLog({
      time: new Date().toLocaleTimeString(),
 ...
 */
export const isTodayAvailable = (lastTimeSec: number) => {
 ...
 */
export const pickArenaTargetId = (targets: ArenaTargets) => {
  const candidate =
    targets?.rankList?.[0] ||
    targets?.roleList?.[0] ||
    targets?.targets?.[0] ||
