...
interface ScheduledTaskLogData {
  name: string;
...
export function createLogManager({
   ...
   */
  const copyLogs = (message: MessageApiLike) => {
    ...
    navigator.clipboard
      ...
      .catch((err: unknown) => {
        message.error(
          "复制日志失败: " + (err instanceof Error ? err.message : String(err)),
        );
      });
  };
 ...
 */
export const addTaskSaveLog = (
  task: ScheduledTaskLogData,
  isNew: boolean,
  addLog: AddLog,
) => {
  addLog({
