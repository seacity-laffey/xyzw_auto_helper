...
interface CarInfo extends LooseRecord {
  key?: number;
  id?: string | number;
  color?: number | string;
  rewards?: RewardItem[];
  sendAt?: number | string;
  refreshCount?: number | string;
}
...
type TokenStatus = Record<string, string>;
type StopRef = { value: boolean };
type AddLog = (entry: { time: string; message: string; type: string }) => void;
...
interface ConnectionManagerLike {
  ensureConnection: (tokenId: string, tokens: BatchToken[]) => Promise<unknown>;
  closeConnection: (tokenId: string, tokenName?: string) => void;
}

interface BatchSettingsLike {
...
const readItemQuantity = (payload: LooseRecord, itemId: number) => {
  const role = payload.role as LooseRecord | undefined;
  const items = role?.items as Record<number, { quantity?: number | string }> | undefined;
  return Number(items?.[itemId]?.quantity || 0);
};
 ...
 */
export const normalizeCars = (raw: unknown): CarInfo[] => {
  const r = (raw || {}) as LooseRecord;
  const body = ((r.body as LooseRecord | undefined) || r) as LooseRecord;
  const roleCar = ((body.roleCar as LooseRecord | undefined) ||
    (body.rolecar as LooseRecord | undefined) ||
    {}) as LooseRecord;

  // 优先从 roleCar.carDataMap 解析（id -> info）
  const carMap = roleCar.carDataMap || roleCar.cardatamap;
  if (carMap && typeof carMap === "object") {
    return Object.entries(carMap as LooseRecord).map(([id, info], idx) => ({
      key: idx,
      id,
      ...((info || {}) as LooseRecord),
    }));
  ...
  if (Array.isArray(body) && Array.isArray(arr) && arr.length === 0) arr = body;
 ...
 * @returns {string} - 品质标签
 */
export const gradeLabel = (color: number | string | undefined) => {
 ...
 */
export const isBigPrize = (rewards: unknown) => {
  if (!Array.isArray(rewards)) return false;
  const rewardItems = rewards as RewardItem[];
 ...
 */
export const countRacingRefreshTickets = (rewards: unknown) => {
  if (!Array.isArray(rewards)) return 0;
  return (rewards as RewardItem[]).reduce(
    (acc, r) =>
...
const checkRewardConditions = (
  ...
  (rewards as RewardItem[]).forEach((r) => {
 ...
 */
export const shouldSendCar = (
  car: CarInfo,
  tickets: number,
  minColor = 4,
  customConditions: RewardConditions = {
    gold: 0,
    recruit: 0,
    jade: 0,
    ticket: 0,
 ...
 */
export const canClaim = (car: CarInfo) => {
 ...
 */
export function createCarManager({
  tokenStore,
  connectionManager,
  batchSettings,
  addLog,
  ...
  const smartSendCar = async (
    ...
    try {
      ...
      try {
        ...
        refreshTickets = readItemQuantity(roleRes, 35002);
        addLog({
          time: new Date().toLocaleTimeString(),
      ...
      for (const car of carList) {
        ...
        try {
          ...
          const customConditions: RewardConditions = {
            gold: batchSettings.smartDepartureGoldThreshold || 0,
            recruit: batchSettings.smartDepartureRecruitThreshold || 0,
            jade: batchSettings.smartDepartureJadeThreshold || 0,
            ticket: batchSettings.smartDepartureTicketThreshold || 0,
          };
          ...
          while (shouldRefresh && !shouldStop.value) {
            ...
            const body = resp.body as LooseRecord | undefined;
            const data = ((resp.car as LooseRecord | undefined) ||
              (body?.car as LooseRecord | undefined) ||
              resp) as LooseRecord;
            ...
            if (data && typeof data === "object") {
              ...
              if (Array.isArray(data.rewards)) car.rewards = data.rewards as RewardItem[];
            }
          ...
          addLog({
            time: new Date().toLocaleTimeString(),
            message: `${token.name} 车辆[${gradeLabel(car.color)}]处理失败: ${getErrorMessage(carError)}，跳过该车辆`,
            type: "error",
          });
          continue;
        }
      ...
      addLog({
        time: new Date().toLocaleTimeString(),
        message: `智能发车失败: ${getErrorMessage(error)}`,
        type: "error",
      });
    } finally {
   ...
   */
  const claimCars = async (
    tokenId: string,
    token: BatchToken,
    tokens: BatchToken[],
    tokenStatus: TokenStatus,
    shouldStop: StopRef,
    CarresearchItem: number[],
  ) => {
    try {
      addLog({
        time: new Date().toLocaleTimeString(),
      ...
      let carList = normalizeCars(res?.body ?? res);
      const roleCar = res.roleCar as LooseRecord | undefined;
      const research = roleCar?.research as Record<number, number> | undefined;
      let refreshlevel = research?.[1] || 0;
      ...
      for (const car of carList) {
        ...
        if (canClaim(car)) {
          try {
            ...
            let refreshpieces = readItemQuantity(roleRes, 35009);
            while (
              refreshlevel < CarresearchItem.length &&
              refreshpieces >= (CarresearchItem[refreshlevel] || 0) &&
              !shouldStop.value
            ) {
              try {
                await tokenStore.sendMessageWithPromise(
                ...
                refreshpieces = readItemQuantity(updatedRoleRes, 35009);
                ...
                addLog({
                  time: new Date().toLocaleTimeString(),
                  message: `${token.name} 车辆改装升级失败: ${getErrorMessage(e)}`,
                  type: "error",
                });
                break; // 升级失败时跳出循环
              }
            ...
            addLog({
              time: new Date().toLocaleTimeString(),
              message: `${token.name} 收车失败: ${getErrorMessage(e)}`,
              type: "warning",
            });
          }
          await new Promise((r) => setTimeout(r, 300));
      ...
      addLog({
        time: new Date().toLocaleTimeString(),
        message: `${token.name} 收车失败: ${getErrorMessage(error)}`,
        type: "error",
      });
    } finally {
      closeConnection(tokenId, token.name);
